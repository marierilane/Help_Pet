...
//
    ...
    // metodo atualizar ocorrencia
    public Ocorrencia update(Long id, Ocorrencia ocorrencia1){
        Ocorrencia ocorrencia = ocorrenciaRepository.findById(id).orElse(null);
        if(ocorrencia != null){
            Usuario usuario = usuarioService.getUsuarioLogado();
            if(usuario.getPapeis().contains(Papel.USUARIO_NORMAL) && ocorrencia.getStatus().equals(Status.PENDENTE)){

                ocorrencia1.setId(ocorrencia.getId());
                ocorrencia1.setTitulo(ocorrencia.getTitulo());
                ocorrencia1.setDescricao(ocorrencia.getDescricao());
                ocorrencia1.setEndereco(ocorrencia.getEndereco());
                ocorrencia1.setOng(ocorrencia.getOng());
                ocorrencia1.setStatus(ocorrencia.getStatus());
                ocorrencia1.setAnimal(ocorrencia.getAnimal());
                ocorrencia1.setPessoa(ocorrencia.getPessoa());

                return ocorrenciaRepository.saveAndFlush(ocorrencia1);
            }else{
                throw new DataIntegrityException("Ocorrência de outre pessoa ou não está mais no status de pendente");
            }
        }else{
        throw new NotFoundException("Ocorrência não encontrada!");
        }
